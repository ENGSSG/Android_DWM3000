...
#define PLATFORM_ID_DW3000  0 /* DW3000 (no PDOA) platform ID. */
#define PLATFORM_ID_DW3001C 1 /* DW3001C (no PDOA) platform ID. */
...
/* On DW3001C_PVT2, use the characterized common channel 5 antenna delay. */
...
enum qerr l1_config_platform_ops_reset_to_default(void)
{
    ...
    if (r)
        return r;
    otp_rev = val32 & 0xFF;
    cal_rev = (val32 >> 8) & 0xFF;
    platform_id = val32 >> 16;

    /* Support both QM331x modules and DW3000/DW3001C OTP layouts on the nRF52840 host. */
    if (platform_id == PLATFORM_ID_QM33120)
        support_aoa = true;
    else if ((platform_id == PLATFORM_ID_QM33110) || (platform_id == PLATFORM_ID_DW3000) ||
             (platform_id == PLATFORM_ID_DW3001C))
        support_aoa = false;
    else
        return QERR_SUCCESS;
